...
For this example, we can leave the component and service as they are. However, before deploying our service, we need to _build_ its component. Building a component basically implies packing it together with its dependencies.
...
Which will return us an URL to browse the service we just deployed. The `-i` flag is used to generate automatically an HTTP inbound with a random domain and link it to our service.
